...
public class IncidentStatusUpdateListener {
  ...
  public void handleIncidentClosed(IncidentClosedEvent event) {
    ...
    log.info("Updating incident with Slack channel {} to RESOLVED", event.slackChannel());

    IncidentEntity incident =
        incidentPersistencePort
            .findBySlackChannelId(event.slackChannel())
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "No incident found for Slack channel " + event.slackChannel()));

    incidentPersistencePort.updateIncidentStatus(incident.getId().toString(), Status.RESOLVED);
