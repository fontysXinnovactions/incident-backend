...
public class SlackIncidentClosureBroadcaster implements IncidentClosurePort {
  ...
  public void closeIncident(String developerUserId, String channelId, String reason) {
    try {
      ...
      // extract reporter information from a channel topic
      ...
      // notify an original reporter in workspace A
  ...
  private void removeAllMembers(String channelId) {
    try {
      ConversationsMembersResponse membersResponse =
          Slack.getInstance()
              .methods(botTokenB)
              .conversationsMembers(req -> req.channel(channelId));

      managerBotMessagingPort.sendMessage(
          channelId, "Removing all members from channel " + channelId);

      if (!membersResponse.isOk()) {
        log.error(
            "Failed to get members for channel {}: {}", channelId, membersResponse.getError());
        return;
      }
      ...
      for (String member : members) {
        try {
          ConversationsKickResponse kickResponse =
              Slack.getInstance()
                  .methods(botTokenB)
                  .conversationsKick(req -> req.channel(channelId).user(member));
          ...
          if (!kickResponse.isOk()) {
            log.warn(
                ...
                member,
                channelId,
                kickResponse.getError());
          } else {
            log.info("Removed member {} from channel {}", member, channelId);
          }
        } catch (IOException | SlackApiException e) {
          log.warn(
              "Error removing member {} from channel {}: {}", member, channelId, e.getMessage());
        }
      }

    } catch (IOException | SlackApiException e) {
      log.error("Error getting members for channel {}: {}", channelId, e.getMessage(), e);
    }
  }
