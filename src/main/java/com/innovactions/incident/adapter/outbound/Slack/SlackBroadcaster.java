...
public class SlackBroadcaster implements IncidentBroadcasterPort {
  ...
  public String broadcast(Incident incident, Platform platform) {
    ...
    try {
      ...
      ConversationsCreateResponse createResponse =
          Slack.getInstance()
              .methods(botToken)
              .conversationsCreate(req -> req.name(channelName).isPrivate(false));

      if (!createResponse.isOk()) {
        log.error(
            "Failed to create channel for incident [{}]: {}",
            incident.getId(),
      ...
      ConversationsSetTopicResponse topicResponse =
          Slack.getInstance()
              .methods(botToken)
      ...
      ConversationsInviteResponse inviteResponse =
          Slack.getInstance()
              .methods(botToken)
              .conversationsInvite(req -> req.channel(channelId).users(List.of(developerUserId)));

      if (!inviteResponse.isOk()) {
        log.warn(
            "Failed to invite developer to channel {}: {}", channelName, inviteResponse.getError());
      } else {
      ...
      Slack.getInstance()
          .methods(botToken)
          .chatPostMessage(req -> req.channel(channelId).text(incident.summary()));
      ...
      log.error(
          "Failed to broadcast incident [{}] to Slack: {}", incident.getId(), e.getMessage(), e);
      return null;
    }
  }
