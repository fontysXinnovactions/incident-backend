...
//  private final EncryptionService encryptionService;
  ...
  public String initSlackDeveloperWorkspace(Incident incident, Platform platform) {
    ...
    channelAdministrationPort.setChannelTopic(
        channelId, "reporterid:" + encryptedReporterId + "_" + platform);
  ...
  public void updateIncidentToDeveloper(Incident incident, String channelId) {
    if (channelId == null) {
      // TODO: prompt the user to ask for the incident ID so we can link to it and update the
      // existing channel
    ...
    try {
      ...
      managerBotMessagingPort.sendMessage(
          channelId, "🔄 Incident update at " + formattedNow + "\n" + incident.getDetails());
