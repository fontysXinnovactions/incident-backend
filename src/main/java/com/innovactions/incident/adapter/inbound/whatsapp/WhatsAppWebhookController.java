...
/**
 * Handles incoming webhooks from the WhatsApp Cloud API.<p>
 * This controller serves two purposes:<br>
 *  - Webhook verification (GET) — allows Meta to verify the endpoint using a token challenge.<br>
 *  - Message processing (POST) — receives user messages and forwards them into the incident workflow.
 */
@Slf4j
@RestController
...
public class WhatsAppWebhookController {
     ...
     *
     * <p>When Meta verifies your webhook URL, it sends a GET request with query parameters:
     * <code>hub.mode</code>, <code>hub.verify_token</code>, and <code>hub.challenge</code>.</p>
     *
     * <p>If the mode is <code>subscribe</code> and the 'verify' token matches,
     * you must respond with the challenge string.</p>
     */
    @GetMapping
     ...
     */
    @PostMapping
    public ResponseEntity<Void> receiveWebhook(@RequestBody(required = false) WhatsAppPayload payload) {
        ...
        try {
            ...
            log.info("Successfully processed WhatsApp message from incident reporter {}", command.reporterId());
