    ...
    public void register(App app) {
        ...
        app.blockAction("report_bug", (req, ctx) -> {
            ...
            reporterBotMessagingPort.sendMessage(
                userId, 
                "<@" + userId + ">, please describe the bug in detail.\n" +
        ...
        app.blockAction("check_status", (req, ctx) -> {
            ...
            String isUserUpdating = pendingReportState.isUpdating(userId) ? "updating" : "not updating";
            reporterBotMessagingPort.sendMessage(userId, "<@" + userId + ">, your report is currently " + isUserPending + " and is " + isUserUpdating + ".");
            return ctx.ack();
        });

        // update_incident --> prompt user to provide updated details
        app.blockAction("update_incident", (req, ctx) -> {
            String userId = req.getPayload().getUser().getId();
            pendingReportState.markUpdating(userId);
            reporterBotMessagingPort.sendMessage(
                userId, 
                "<@" + userId + ">, please provide some updated details of your incident."
            );
            return ctx.ack();
        });
    ...
    app.event(
        ...
        (payload, ctx) -> {
            ...
            if (pendingReportState.isPending(userId)) {
                ...
                CompletableFuture.runAsync(() -> {
                    try {
                        CreateIncidentCommand updateCommand = new CreateIncidentCommand(
                                userId,
